...
async function submit() {
  ...
  try {
    const response = await axios.post(`${config.public.apiBaseUrl}/create-shop`, formData, {
      headers: {
...
<template>
  <UModal v-model="isOpen">
    <div class="relative pt-0 p-6">
      ...
      <UForm :state="state" class="space-y-4" :schema="schema" @submit="checkUsernameAvailabilty">
        <UFormGroup label="Shop Name" required name="name">
          <UInput v-model="state.name" />
        </UFormGroup>
        <UFormGroup label="Shop Unique Name" description="This will be used for your sharable Link" required name="username">
